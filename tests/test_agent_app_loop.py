...
from agent_app.core.loop import run_agent_loop
from agent_app.core.prompt import PromptBuilder
from agent_app.core.recovery import PartialStreamError
from agent_app.features.background import BackgroundState
from agent_app.features.mcp import MCPState
from agent_app.features.memory import MemoryStore
from agent_app.features.scheduler import CronJob, SchedulerState
from agent_app.features.skills import SkillState
from agent_app.features.tasks import TaskStore
from agent_app.features.teams.bus import MessageBus
from agent_app.features.teams.protocol import ProtocolStore
from agent_app.features.teams.teammates import TeamState
from agent_app.features.worktrees import WorktreeState
from agent_app.runtime import RuntimeContext, SessionState
from agent_app.tools.hooks import HookRegistry
...
def test_compact_is_control_flow_and_skips_later_tools(runtime_factory, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent_app.core.loop.compact_history",
...
def test_prompt_too_long_compacts_once_then_rebuilds_request(
    ...
    monkeypatch.setattr(
        "agent_app.core.loop.reactive_compact", fake_compact
    )
...
def test_background_dispatch_uses_handler_snapshot_without_real_thread(
    ...
    monkeypatch.setattr(
        "agent_app.core.loop.start_background_task", fake_start
    )
...
def test_background_completion_after_dispatch_is_appended_as_text(
    ...
    monkeypatch.setattr(
        "agent_app.core.loop.start_background_task",
...
def test_finalization_runs_stop_then_team_wait_then_memory(
    ...
    monkeypatch.setattr(
        "agent_app.core.loop._wait_for_team_activity",
        lambda _runtime: events.append("team") or False,
    )
    monkeypatch.setattr(
        "agent_app.core.loop.extract_memories",
        lambda *_args: events.append("extract"),
    )
    monkeypatch.setattr(
        "agent_app.core.loop.consolidate_memories",
